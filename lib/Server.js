...
global.__reactNativeGlobalProps = require("react-native-global-props");\
global.__reactNativeVideo = require("react-native-video");\
global.__reactNativeFetchBlob = require("react-native-fetch-blob");\
';
const SOURCEMAP_REGEX = /\/\/[#@] sourceMappingURL=([^\s'"]*)/;

function reactImportTransform(context, request, callback) {
  switch (request) {
  case 'react': return callback(null, '__react');
  case 'react-native': return callback(null, '__reactNative');
  case 'react-native-global-props': return callback(null, '__reactNativeGlobalProps');
  case 'react-native-video': return callback(null, '__reactNativeVideo');
  case 'react-native-fetch-blob': return callback(null, '__reactNativeFetchBlob');
  }
...
class Server {
  ...
  start() {
    ...
    this.appliedProjectRoots = this.projectRoots || [];
    this.appliedProjectRoots.unshift(this.entryDir);
    ...
    return this._writeEntryFiles().then(() => {
      ...
      this.server = express()
        ...
        .use(getDevToolsMiddleware({port: this.port, projectRoots: this.appliedProjectRoots}, () => wsProxy && wsProxy.isChromeConnected()))
        .use(getDevToolsMiddleware({port: this.port, projectRoots: this.appliedProjectRoots}, () => ms && ms.isChromeConnected()))
        .use(openStackFrameInEditorMiddleware({projectRoots: this.appliedProjectRoots}))
      ...
      const listenPromise = new Promise(resolve => {
        const serverInstance = this.httpServer = this.server.listen(this.port, () => {
  ...
  _startPackageServer() {
    ...
    return new Promise((resolve, reject) => {
      ...
      const args = script.concat([
        '--port', this.packagerPort,
        '--projectRoots', this.appliedProjectRoots.concat(additionalRoots).join(','),
      ]).concat(
